...
import {
  findUserByEmail, findUserByMobile, createUser, upsertEmailOtp,
  findActiveOtp, findOtpForVerification, markOtpAsUsed, verifyUserEmail,
  createRefreshToken, findUserByIdentifier, findRefreshToken, revokeRefreshToken,
  revokeAllRefreshTokens, updateUserProfile, updateUserPassword, createGoogleUser,
  findUserByGoogleId, updateGoogleProfile, findUserById, linkGoogleAccount
...
export const registerUser = async (registerData: RegisterInput) => {
  ...
  if (existingUserByEmail || existingUserByMobile) {
    ...
    const existingOtp = await findActiveOtp(email, OtpPurpose.registration);
    if (existingOtp) {
      const now = new Date();
      const cooldownEndsAt = new Date(existingOtp.lastSentAt.getTime() + 30 * 1000);
      if (now < cooldownEndsAt) throw new AppError("Please wait before requesting another OTP", 429);
      if (existingOtp.resendCount >= 5) throw new AppError("Too many OTP requests. Please try again after 15 minutes", 429);
    }

    const otp = generateOtp();
    await upsertEmailOtp(email, OtpPurpose.registration, otp, getOtpExpiry());
...
export const verifyEmail = async (verifyData: VerifyEmailInput) => {
  ...
  const emailOtp = await findOtpForVerification(email, otp, OtpPurpose.registration);
...
export const completeProfile = async (userId: string, profileData: CompleteProfileInput) => {
  ...
  if (user.provider === "google") {
    if (!profileData.mobileNumber) throw new AppError("Mobile number is required", 400);
    if (!profileData.password) throw new AppError("Password is required", 400);

    const existingMobile = await findUserByMobile(profileData.mobileNumber);
    ...
    updatedUser = await updateGoogleProfile(userId, {
...
export const forgotPassword = async (forgotData: ForgotPasswordInput) => {
  ...
  const existingOtp = await findActiveOtp(email, OtpPurpose.forgotPassword);
  if (existingOtp) {
    const cooldownEndsAt = new Date(existingOtp.lastSentAt.getTime() + 30 * 1000);
  ...
  const otp = generateOtp();
  await upsertEmailOtp(email, OtpPurpose.forgotPassword, otp, getOtpExpiry());
  await sendOtpEmail(email, otp);
...
export const verifyForgotPasswordOtp = async (verifyData: VerifyForgotPasswordOtpInput) => {
  const { email, otp } = verifyData;
  const emailOtp = await findOtpForVerification(email, otp, OtpPurpose.forgotPassword);
...
export const googleLogin = async (googleData: GoogleLoginInput) => {
  ...
  let user = await findUserByGoogleId(uid);
  ...
  if (!user) {
    ...
    if (existingEmailUser) {
      if (existingEmailUser.provider === "email") {
        user = await linkGoogleAccount(existingEmailUser.id, uid);
      } else {
