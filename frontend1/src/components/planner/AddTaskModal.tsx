...
import { Modal, View, Text, TouchableOpacity, StyleSheet, Platform, Pressable, KeyboardAvoidingView, ScrollView } from "react-native";
...
type AddTaskModalProps = { visible: boolean; onClose: () => void; };
...
const AddTaskModal = ({ visible, onClose }: AddTaskModalProps) => {
  ...
  const { control, handleSubmit, setValue, watch, reset, formState: { errors, isSubmitting } } = useForm<CreatePlannerTaskForm>({
  ...
  const onSubmit = async (data: CreatePlannerTaskForm) => {
    await createTask({ title: data.title, description: data.description, dueDate: data.dueDate });
    reset();
    onClose();
  };
          ...
          <View style={styles.container} onStartShouldSetResponder={() => true}>
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <TouchableOpacity style={styles.dateButton} onPress={() => setShowDatePicker(true)}>
                ...
                <Text style={styles.dateText}>{new Date(dueDate).toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })}</Text>
              </TouchableOpacity>
              ...
              <View style={styles.footer}>
                <CustomButton title="Create Task" loading={isSubmitting} onPress={handleSubmit(onSubmit)} />
              </View>
